              ...
              {message.role === 'onboarding' && message.onboardingData ? (
                <div className="flex items-start justify-start">
                  <div className="bg-transparent text-slate-900 w-full min-w-0 overflow-hidden">
                    ...
                    {message.content && (
                      <div className="whitespace-pre-wrap break-words" style={{ fontSize: '15px', lineHeight: '17px' }}>
                        {message.content}
                      </div>
                    )}
                ...
                <div className={`flex items-start ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {message.role === 'assistant' ? (
                    <div className="bg-transparent text-slate-900 w-full min-w-0 overflow-hidden">
                      {!isGroupedWithPrev && renderAssistantHeader()}
                      <div className="whitespace-pre-wrap break-words" style={{ fontSize: '15px', lineHeight: '17px' }}>{message.content}</div>
              ...
              >
                  <div className="bg-white border border-[#EBEBEB] rounded-xl shadow-[0px_4px_16px_0px_rgba(0,0,0,0.08)] flex flex-col">
                    <textarea
                      ...
                      className="w-full bg-transparent px-[16px] py-3 text-[15px] leading-[22px] text-slate-900 focus:outline-none resize-none chat-input-placeholder overflow-y-auto"
        ...
        <div className="bg-white px-6 pb-[8px] flex justify-center shrink-0 sticky bottom-0 z-10">
          ...
          <div className="bg-white border border-[#EBEBEB] rounded-xl shadow-[0px_4px_16px_0px_rgba(0,0,0,0.08)] flex flex-col">
            <textarea
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(); } }}
              placeholder={t('ai.inputPlaceholder')}
              className="w-full bg-transparent p-3 text-[15px] leading-[22px] text-slate-900 focus:outline-none resize-none chat-input-placeholder overflow-y-auto"
                      style={{ minHeight: '72px', maxHeight: '144px', height: '72px' }}
                      onInput={(e) => { const el = e.currentTarget; el.style.height = '72px'; el.style.height = Math.min(el.scrollHeight, 144) + 'px'; }}
            />
            <div className="flex items-center justify-between px-3 pb-3">
