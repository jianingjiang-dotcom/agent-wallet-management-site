  ...
  if (status === 'completed') {
    return (
      <div className="bg-[var(--app-card-bg)] border border-[rgba(34,197,94,0.3)] rounded-[12px] p-4 transition-all duration-300">
        ...
        <div className="rounded-[12px] bg-[var(--app-bg)] border border-[rgba(10,10,10,0.06)] px-3.5 py-2.5 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="font-['JetBrains_Mono',monospace] text-[13px] font-medium text-[var(--app-text)]">
              ${confirmedPerTx}
            </span>
          </div>
    ...
    <div className="flex items-center gap-2.5 flex-wrap">
      {options.map((val) => (
        <label key={val} className="flex items-center cursor-pointer">
          ...
          />
          <span className="ml-1.5 font-normal text-[13px] text-[var(--app-text)]">${val}</span>
        </label>
      ))}
      ...
      <div className="flex items-center">
        <span className="font-normal text-[13px] text-[var(--app-text-secondary)] mr-1">$</span>
        <input
          type="number" value={customValue}
          onChange={(e) => onCustomChange(e.target.value)}
          onFocus={() => onSelect('custom')}
          placeholder="0"
          className="w-16 bg-[var(--app-card-bg)] border border-[var(--app-border-medium)] rounded-[6px] px-2 py-1 font-normal text-[13px] text-[var(--app-text)] focus:outline-none focus:border-[#1F32D6] focus:shadow-[0px_2px_12px_0px_rgba(31,50,214,0.08)]"
        />
      </div>
    </div>
  );
